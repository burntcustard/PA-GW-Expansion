...
//        });
    ...
    function GameViewModel(game) {
        ...
        // When a card gets added to the main list it should be added to one of these too
        self.cardsOfType = {
            commanderPrimary: ko.observableArray(),
            commanderSecondary: ko.observableArray(),
            commanderPassive: ko.observableArray(),
            units: ko.observableArray(),
            upgrades: ko.observableArray()
        };

        self.cardsOfTypeArray = ko.observableArray([
            {
                type: ko.observable('commanderPrimary'),
                name: 'Commander Primary',
                cards: self.cardsOfType.commanderPrimary
            },
            {
                type: ko.observable('commanderSecondary'),
                name: 'Commander Uber',
                cards: self.cardsOfType.commanderSecondary
            },
            {
                type: ko.observable('commanderPassive'),
                name: 'Commander Chassis',
                cards: self.cardsOfType.commanderPassive
            },
            {
                type: ko.observable('units'),
                name: 'Units & Structures',
                cards: self.cardsOfType.units
            },
            {
                type: ko.observable('upgrades'),
                name: 'Mods & Upgrades',
                cards: self.cardsOfType.upgrades
            }
        ]);
        ...
        var updateCards = function() {
            ...
            _.forEach(['commanderPrimary',
                       ...
                       'commanderPassive',
                       'units',
                       'upgrades'], function(type) {
                var cardsOfTypeX = inventory.cardsOfType[type];
                var cardModelsOfTypeX = self.cardsOfType[type]();
                var numCardsOfType = inventory.maxCardsOfType()[type];
                // console.log(type);
                // console.log(cardsOfTypeX);
                // console.log(cardModelsOfTypeX);
                // console.log(numCardsOfType);
                // console.log(cardModelsOfTypeX.length);
                if (numCardsOfType < cardModelsOfTypeX.length) {
                    self.cardsOfType[type].splice(numCardsOfType, cardModelsOfTypeX.length);
                } else {
                    while (numCardsOfType > cardModelsOfTypeX.length) {
                        self.cardsOfType[type].push(new CardViewModel(cardsOfTypeX[cardModelsOfTypeX.length]));
                    }
                }

                _.forEach(cardModelsOfTypeX, function(model, index) {
                    var card = cardsOfTypeX[index];
                    if (!_.isEqual(card, model.params()))
                        model.params(card);
                });
            });
            ...
            // _.forEach(self.cardsOfType, function(cardModelsOfTypeX) {
            //     console.log("Card model of type: ");
            //     console.log(cardModelsOfTypeX());
            // });
            //
            // console.log("Final cards");
            // console.log("Main card list:");
            // console.log(cards);
            // _.forEach(inventory.cardsOfType, function(cardsOfTypeX) {
            //     console.log("Cards of type: ");
            //     console.log(cardsOfTypeX);
            // });
